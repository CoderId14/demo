...
public class BookWriterService {
    ...
    public PagedResponse<BookWriterResponse> search(BookWriterRequestInput input, Pageable pageable, CustomUserDetails currentUser) {
        ...
        List<BookWriterResponse> result = bookWriterDTOS.getContent().stream().map(BookWriterResponse::from).toList();
        PagedResponse<BookWriterResponse> response = new PagedResponse<>(result,
